...
class MainWindow(myLayout):
    def __init__(self):
        ...
        modelPath = str(Path().resolve().parent / 'model' / 'res10_300x300_ssd_iter_140000.caffemodel')
        self.detector = cv2.dnn.readNetFromCaffe(protoPath, modelPath)
        predictor = dlib.shape_predictor(str(Path().resolve().parent / 'model' / 'shape_predictor_68_face_landmarks.dat'))
    ...
    def resizeVideo(self):
        try:
            ...
            QMessageBox.information(self, "QMessageBox.information()", 'start camera first!')  
    ...
    def snapshot(self):
        try:
            ...
            cv2.imwrite(str(filePath), cv2.cvtColor(self.frame, cv2.COLOR_RGB2BGR))
            QMessageBox.information(self, "QMessageBox.information()", f'image is saved as: {filename}.jpg')
        except:
            QMessageBox.information(self, "QMessageBox.information()", 'start camera first!')
    ...
    def recording(self):
        ...
        if self.writer is None:
            ...
            filePath = Path().resolve().parent / 'output' / f'{self.videoName}.mp4'
            self.writer = cv2.VideoWriter(str(filePath), fourcc, 10, (self.frame.shape[1], self.frame.shape[0]), True)
    ...
    def addFace(self):
        if not self.cap and len(self.frame) == 0:
            ...
            QMessageBox.information(self, "QMessageBox.information()", 'please stop face recognition process!')
        else:
            ...
            cv2.imwrite(str(filePath), cv2.cvtColor(self.frame, cv2.COLOR_RGB2BGR))
            QMessageBox.information(self, "QMessageBox.information()", f'{filename} is added to database')
    ...
    def loadFaces(self):
        ...
        for name in self.knownNames:
            self.box_colors.update({name: (np.random.randint(128,255), np.random.randint(128,255), np.random.randint(128,255))})
        self.box_colors.update({'siapa?': (255,0,0)})


    def alignedFace(self):
        gray = cv2.cvtColor(self.frame, cv2.COLOR_BGR2GRAY)
        dlibrect = dlib.rectangle(int(self.startX), int(self.startY), int(self.endX), int(self.endY))
        ...
        if len(detections) > 0:
            ...
            for i in range(0, detections.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    face_distances = face_recognition.face_distance(self.knownEmbeddings, face_encoding)
                    ...
                    # bounding box
                    sX, sY, eX, eY = int(self.startX*rw), int(self.startY*rh), int(self.endX*rw), int(self.endY*rh)
                    cv2.rectangle(self.frame_ori, (sX, sY), (eX, eY), self.box_colors[name], 2)
                    # put the name
                    ...
                    cv2.rectangle(overlay, box_coords[0], box_coords[1], self.box_colors[name], -1)
                    cv2.putText(overlay, text, (text_offset_x, text_offset_y), font, fontScale=font_scale, color=(0,0,0), thickness=2)
                    # apply the overlay
                    alpha=0.6
                    cv2.addWeighted(overlay, alpha, self.frame_ori, 1 - alpha, 0, self.frame_ori)
    ...
    def controlTimer(self):
        ...
        if not self.timer.isActive():
            self.startBtn.setStyleSheet("QPushButton {background-color: #DE3163}")
        ...
        # if timer is started
            ...
            if self.detectTimer.isActive():
                ...
                self.startBtn.setStyleSheet("QPushButton {background-color: #f9e79f}")
                self.timer.stop()
                self.cap.release()
                self.cap = None
    ...
    def controlRecorder(self):
        ...
        if not self.recordTimer.isActive():
            if not self.cap:
                ...
                self.recordBtn.setStyleSheet("QPushButton {background-color: #DE3163}")
                self.recordTimer.start(20)
                self.recordBtn.setText("Stop Recording")
        # if timer is started
        else:
            self.recordBtn.setStyleSheet("QPushButton {background-color: #f9e79f}")
            self.recordTimer.stop()
            self.recordBtn.setText("Start Recording")
            self.writer = None
            QMessageBox.information(self, "QMessageBox.information()", f'video is saved as: {self.videoName}.mp4')
    ...
    def controlDetector(self):
        ...
        if not self.detectTimer.isActive():
            if not self.cap:
                ...
                QMessageBox.information(self, "QMessageBox.information()", 'database is empty')
            else:
                self.detectBtn.setStyleSheet("QPushButton {background-color: #DE3163}")
                # start detection
        ...
        # if timer is started
            ...
            self.detectBtn.setStyleSheet("QPushButton {background-color: #f9e79f}")
            # stop detection
