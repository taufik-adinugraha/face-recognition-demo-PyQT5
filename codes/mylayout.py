...
class myLayout(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f'Face Recognition by Taufik Adi Nugraha')
        self.resize(700,400)
        self.width = 900
        ...
        faceIdx = cv2.imread(str(Path().resolve().parent / 'images' / 'facial.png'))
        ...
        self.faceName.setPlaceholderText('fulan')
        ...
        # image/video
        self.image = QLabel()
        imgIdx = cv2.imread(str(Path().resolve().parent / 'images' / 'index.png'))
    ...
    def launchViewFolder(self):
        ...
        list_files = [i.split('.')[0] for i in list_files if i.split('.')[1] in ['jpg', 'mp4']]
        if len(list_files) == 0:
            QMessageBox.information(self, "QMessageBox.information()", 'folder is empty')
        else:
...
class viewFolder(QDialog):
    ...
    def deleteFile(self):
        ...
        if name != 'None':
            ...
            QMessageBox.information(self, "QMessageBox.information()", 'please select a file!')
